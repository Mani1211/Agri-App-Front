...
searchKey1: string;
 ...
 @ViewChild(MatPaginator) paginator3:MatPaginator
  ...
  constructor(private router:Router,private balerRent:BalerService,private balerSell:BalersellService) { }
  ...
  displayedColumns1: string[] = ['date','customerName','customerPlace','billNumber','numberOfBales','rentOfBales','totalAmount','advance','balance','amountGiven'];

  displayedColumns2: string[] = ['date','buyerName','buyerCellNumber','vehicleNumber','numberOfBales','costPerBales','totalAmount','advance','balance','amountGiven'];

  displayedColumns3: string[] = ['date','driverName','managerName','driverSalary','managerSalary','workerSalary','petrol','diesel','service','spare','juteCost','hayCost','foodCost','totalAmount'];
  ...
  fetchRentIncomes(){
    this.balerRent.getIncomeDetails().subscribe((data:any) =>{
   ...
   fetchSellIncomes(){
    this.balerSell.getIncomeDetails().subscribe((data:any) =>{
   ...
   fetchExpense(){
    this.balerRent.getExpenseDetails().subscribe((data:any) =>{
  ...
  totalrentBales(){
    for(let i=0;i<this.rentIncomeDetails.length;i++){
      this.totalRentBales = this.totalRentBales + this.rentIncomeDetails[i].numberOfBales;
    }
  ...
  totalsellBales(){
    for(let i=0;i<this.sellIncomeDetails.length;i++){
      this.totalSellBales = this.totalSellBales + this.sellIncomeDetails[i].numberOfBales;
    }
  ...
  totalIncomeRent(){
    for(let i=0;i<this.rentIncomeDetails.length;i++){
      this.totalRentIncome = this.totalRentIncome + this.rentIncomeDetails[i].amountGiven;
    }
  ...
  totalIncomeSell(){
    for(let i=0;i<this.sellIncomeDetails.length;i++){
      this.totalSellIncome = this.totalSellIncome + this.sellIncomeDetails[i].amountGiven;
    }
    return console.log(this.totalRentIncome);
  }  
  ...
  totalBalanceRent(){
    for(let i=0;i<this.rentIncomeDetails.length;i++){
      this.totalRentBalance = this.totalRentBalance + this.rentIncomeDetails[i].balance;
    }
    return console.log(this.totalRentBalance);
  }  
  totalBalanceSell(){
    for(let i=0;i<this.sellIncomeDetails.length;i++){
      this.totalSellBalance = this.totalSellBalance + this.sellIncomeDetails[i].balance;
    }
