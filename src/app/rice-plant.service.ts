...
export class RicePlantService {
  ...
  updateRiceIncome( id,date,customerName,customerPlace,billNumber,numberOfAcre,costPerAcre,totalAmount,advance,balance,amountGiven){
    const income ={
      id:id,
    date:date,
    customerName:customerName,
  ...
  addRiceExpense(driverName,managerName,driverSalary,managerSalary,
...
return this.http.post(`${this.uri}/riceexpensedetails/add`,expense)
  ...
  updateRiceExpense(id,driverName,managerName,driverSalary,managerSalary,
    date,petrol,diesel,spare,service,foodCost,totalAmount){
  const expense={
    id:id,
  driverName:driverName,
